...
open class DelaunayTriangulationBuilder {
    ...
    private fun create() {
        ...
        val vertices = toVertices(siteCoords!!)
        val sd = QuadEdgeSubdivision(siteEnv, tolerance)
        subdiv = sd
    ...
    companion object {
        ...
        fun unique(coords: Array<Coordinate>): CoordinateList {
            val coordsCopy = CoordinateArrays.copyDeep(coords)
            coordsCopy.sort()
            val coordList = CoordinateList(coordsCopy, false)
            return coordList
        }
